...
public class PotentialServiceImpl implements PotentialService {
    ...
    public PotentialParameters computePotential(PotentialDto p) {
        if (p.r() == null || p.r() <= 0)
            throw new IllegalArgumentException("r > 0 required");

        if (p.structure() == null)
            throw new IllegalArgumentException("Structure type required");
        ...
        double re = (p.re() != null)
                ? p.re()
                : nn * LatticePhysics.morseReFactor(st);

        double sigma = (p.sigma() != null)
                ? p.sigma()
                : re / Math.pow(2, 1.0 / 6.0);

        double epsilon = (p.epsilon() != null)
                ? p.epsilon()
                : 1e-21 * LatticePhysics.packingFactor(st);

        double value = switch (p.type()) {
            case MORSE -> {
                if (p.De() == null || p.a() == null)
                    throw new IllegalArgumentException("Morse requires De and a");
                yield morse(p.r(), p.De(), p.a(), re);
            }
            case LENNARD_JONES -> lj(p.r(), sigma, epsilon);
        };
