...
public class ResultController {
    ...
    @GetMapping("/config")
    public ResponseEntity<ApiResponse<List<ResultDTO>>> getAll() {
        List<ResultDTO> dtos = resultService.findAll();
        return ResponseEntity.ok(
                new ApiResponse<>(dtos, "Результаты для конфига получены", HttpStatus.OK.value())
        );
    }
}
