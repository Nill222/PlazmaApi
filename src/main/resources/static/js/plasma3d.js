...
const PLASMA_COLORSCALE = [[0.00, '#0c0033'], [0.10, '#1a0080'], [0.20, '#0000cc'], [0.30, '#0066ff'], [0.40, '#00ccff'], [0.50, '#00ff99'], [0.60, '#66ff33'], [0.70, '#ffff00'], [0.80, '#ff9900'], [0.90, '#ff3300'], [1.00, '#ff0066'],];
...
function _fillGrid(nRows, nCols, value) {
    return Array.from({length: nRows}, () => Array.from({length: nCols}, () => value));
}
...
function addPhysics3DData(stats, simReq) {
    ...
    const hasValidGrid = isFinite(minT) && isFinite(maxT) && (maxT - minT) > 5 && times?.length > 0 && depths?.length > 0;
...
function set3DViewType(type) {
    ...
    window.PlasmaAnimations?.ToastNotifications?.show(`Режим: ${type === 'surface' ? '3D Поверхность' : 'Сфера + пересечение'}`, 'info', 2000);
}
...
function _renderSurface(el, data) {
    ...
    const trace = {
        ...
        hovertemplate: '<b>Глубина:</b> %{x:.2f} нм<br>' + '<b>Время:</b> %{y:.1f} с<br>' + '<b>T:</b> %{z:.1f} K<extra></extra>',
        contours: {
    ...
    Plotly.react(el, [trace], layout, _config())
        ...
        .catch(err => {
            console.error('[3D] Surface error:', err);
            _showEmptyState(el, 'Ошибка отображения поверхности');
        });
}
...
function _renderSphereWithSection(el, data) {
    ...
    const SHELLS = [{relR: 1.00, opacity: 0.12, nLat: 40, nLon: 80}, {
        relR: 0.82, opacity: 0.20, nLat: 36, nLon: 72
    }, {relR: 0.65, opacity: 0.30, nLat: 32, nLon: 64}, {relR: 0.48, opacity: 0.45, nLat: 28, nLon: 56}, {
        relR: 0.32, opacity: 0.65, nLat: 24, nLon: 48
    }, {relR: 0.18, opacity: 0.85, nLat: 18, nLon: 36}, {relR: 0.07, opacity: 1.00, nLat: 12, nLon: 24},];
    ...
    for (const sh of SHELLS) {
        ...
        traces.push({
            ...
            hovertemplate: `<b>Слой ${(sh.relR * 100).toFixed(0)}%R</b><br>` + `T: ${temp.toFixed(0)} K<extra>Сфера</extra>`,
            scene: 'scene',
            showlegend: false,
            name: `R=${(sh.relR * 100).toFixed(0)}%`,
            lighting: {
                ambient: 0.5, diffuse: 0.9, specular: 0.6, roughness: 0.4, fresnel: 0.3
            },
            lightposition: {
                x: 2000, y: 2000, z: 3000
            },
            contours: {
    ...
    const layout = {
        title: {
            text: `<b>🌐 Температурный профиль: T ядра ≈ ${tempAt(0.07).toFixed(0)} K` + ` | T поверхности ≈ ${tempAt(1.0).toFixed(0)} K</b>`,
        ...
        scene: {
            ...
            zaxis: {visible: false},
            camera: {
                eye: {x: 1.8, y: 1.5, z: 1.4}, center: {x: 0, y: 0, z: 0}, up: {x: 0, y: 0, z: 1}
            },
            aspectmode: 'cube',
        ...
        scene2: {
            ...
            xaxis: {visible: false},
            yaxis: {visible: false},
            zaxis: {visible: false}, // Angled view to see the intersection clearly
            camera: {
                eye: {x: 0.2, y: 2.5, z: 1.2}, center: {x: 0, y: 0, z: 0}, up: {x: 0, y: 0, z: 1}
            },
            aspectmode: 'cube',
            aspectratio: {x: 1, y: 1, z: 1},
            bgcolor: 'rgba(0,0,0,0)',
    ...
    Plotly.react(el, traces, layout, _config())
        ...
        .catch(err => {
            console.error('[3D] Sphere error:', err);
            _showEmptyState(el, 'Ошибка сферической визуализации');
        });
}
...
function _rebuildThermalMap(stats, simReq) {
    ...
    const grid = times.map((_, tIdx) => {
        ...
        const timeFactor = phase < 0.30 ? phase / 0.30 : 1.0 - 0.10 * (phase - 0.30);
